...
async def test_cleanup_dry_run_reports_count(db_session, mock_user):
    ...
    old_discarded = Email(
        gmail_id="g-old-disc", subject="Old discarded", user_id=mock_user.id,
        pre_filter_status="discarded", sender_domain="old.com", sender="old@old.com",
        synced_at=old_time,
    )
    # Old review_pending email — should be eligible
    old_pending = Email(
        gmail_id="g-old-pend", subject="Old pending", user_id=mock_user.id,
        pre_filter_status="review_pending", sender_domain="old2.com", sender="old2@old2.com",
        synced_at=old_time,
    )
    # Recent discarded email — should NOT be eligible
    recent_discarded = Email(
        gmail_id="g-recent-disc", subject="Recent discarded", user_id=mock_user.id,
        pre_filter_status="discarded", sender_domain="recent.com", sender="recent@recent.com",
...
async def test_cleanup_execute_deletes_old_emails(db_session, mock_user):
    ...
    old_email = Email(
        gmail_id="g-exec-old", subject="Old to delete", user_id=mock_user.id,
        pre_filter_status="discarded", sender_domain="delete.com", sender="del@delete.com",
        synced_at=old_time,
    )
    recent_email = Email(
        gmail_id="g-exec-recent", subject="Recent keep", user_id=mock_user.id,
        pre_filter_status="review_pending", sender_domain="keep.com", sender="keep@keep.com",
        synced_at=recent_time,
    )
...
async def test_cleanup_does_not_delete_passed_emails(db_session, mock_user):
    ...
    passed_email = Email(
        gmail_id="g-passed-old", subject="Old passed", user_id=mock_user.id,
        pre_filter_status="passed", sender_domain="passed.com", sender="passed@passed.com",
        synced_at=old_time,
    )
