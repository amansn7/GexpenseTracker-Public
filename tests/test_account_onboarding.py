...
async def test_account_associated_items_are_user_owned(db_session):
    ...
    try:
        ...
        categories = (
            (await db_session.execute(select(UserCategory).where(UserCategory.user_id == user_a.id))).scalars().all()
        )
        services = (
            (await db_session.execute(select(UserAIService).where(UserAIService.user_id == user_a.id))).scalars().all()
        )
