...
async def test_delete_account_removes_all_user_data(db_session, mock_user):
    ...
    from app.models import (
        AuditLog, Budget, ClassificationLog, ConnectedAccount, Debt, DeviceToken,
        DuplicatePair, Email, FilterRule, Goal, GoalContribution, LLMSpendTracker,
        Label, RecurringExpense, RefreshTokenBlacklist, SenderRule, Session,
        SyncProgress, SyncState, Transaction, TransactionCorrection,
        TransactionStatus, User, UserAIService, UserCategory,
        UserMerchantOverride, UserProfile, UserRole, UserSettings, UserStatus,
        ClassifierMethod, MerchantAlias, MerchantEntityAlias,
    )
    ...
    user2 = User(
        email="isolated@delete-test.com", role=UserRole.owner, status=UserStatus.active,
    ...
    email_a = Email(gmail_id="g-del-1", subject="Test", sender="s@test.com",
                    sender_domain="test.com", user_id=uid_1)
    email_b = Email(gmail_id="g-del-2", subject="Test2", sender="s2@test.com",
                    sender_domain="test.com", user_id=uid_1)
    db_session.add_all([email_a, email_b])
    await db_session.flush()

    # two transactions (each with unique email_id) needed for duplicate_pair
    txn_a = Transaction(email_id=email_a.id, label=Label.expense.value, amount=500.0,
                         merchant="Test", category="Food",
                         status=TransactionStatus.auto.value,
                         classifier_method=ClassifierMethod.llm.value)
    txn_b = Transaction(email_id=email_b.id, label=Label.expense.value, amount=300.0,
                         merchant="Test2", category="Travel",
                         status=TransactionStatus.auto.value,
                         classifier_method=ClassifierMethod.llm.value)
    ...
    sess = Session(user_id=uid_1, token=b"a" * 32, expires_at=datetime.now(timezone.utc) + timedelta(hours=1))
    umo = UserMerchantOverride(user_id=uid_1, merchant="TestMerchant", category="Food")
    uas = UserAIService(user_id=uid_1, provider="openai", display_name="Test", model_id="gpt-4", encrypted_api_key="enc")
    ...
    db_session.add_all([
        budget, debt, re, sr, fr, sess, umo, uas, uc, ca, ss, tc, sp, lst, al, dt, rtb,
        goal, ma_fin, ma_ent,
    ])
    await db_session.flush()
    ...
    # --- user 2 fixture data ---
    email_u2 = Email(gmail_id="g-iso-1", sender="s@iso.com", sender_domain="iso.com",
                     subject="Isolated", user_id=user2.id)
    db_session.add(email_u2)
    await db_session.flush()

    txn_u2 = Transaction(email_id=email_u2.id, label=Label.expense.value, amount=999,
                          merchant="Isolated", status=TransactionStatus.auto.value,
                          classifier_method=ClassifierMethod.llm.value)
    ...
    assert (await db_session.execute(select(func.count()).select_from(Email).where(Email.user_id == uid_1))).scalar() == 2
    ...
    assert (await db_session.execute(select(Transaction).where(Transaction.id == txn_a.id))).scalar_one_or_none() is None
    assert (await db_session.execute(select(User).where(User.id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(ClassificationLog).where(ClassificationLog.id == cl.id))).scalar_one_or_none() is None
    assert (await db_session.execute(select(DuplicatePair).where(DuplicatePair.id == dp.id))).scalar_one_or_none() is None
    ...
    assert (await db_session.execute(select(RecurringExpense).where(RecurringExpense.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(SenderRule).where(SenderRule.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(FilterRule).where(FilterRule.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(ConnectedAccount).where(ConnectedAccount.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(Session).where(Session.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(UserMerchantOverride).where(UserMerchantOverride.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(UserAIService).where(UserAIService.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(UserCategory).where(UserCategory.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(UserProfile).where(UserProfile.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(UserSettings).where(UserSettings.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(SyncState).where(SyncState.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(TransactionCorrection).where(TransactionCorrection.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(SyncProgress).where(SyncProgress.user_id == uid_1))).scalar_one_or_none() is None
    assert (await db_session.execute(select(LLMSpendTracker).where(LLMSpendTracker.user_id == uid_1))).scalar_one_or_none() is None
    ...
    assert (await db_session.execute(select(Transaction).where(Transaction.id == txn_u2.id))).scalar_one_or_none() is not None
    assert (await db_session.execute(select(Budget).where(Budget.user_id == user2.id))).scalar_one_or_none() is not None
    assert (await db_session.execute(select(ConnectedAccount).where(ConnectedAccount.user_id == user2.id))).scalar_one_or_none() is not None
