...
async def test_undo_keep_shared_domain_decrements_hit_count(db_session, mock_user, monkeypatch):
    ...
    try:
        ...
        rule = (
            await db_session.execute(
                select(FilterRule).where(
                    FilterRule.rule_type == "allowlist_domain", FilterRule.value == domain, FilterRule.source == "user"
                )
            )
        ).scalar_one_or_none()
        assert rule is None
...
async def test_undo_unreviewed_email_returns_200_noop(db_session, mock_user):
    ...
    email = Email(
        gmail_id="g-undo-never",
        subject="Never reviewed",
        user_id=mock_user.id,
        pre_filter_status="review_pending",
        sender_domain="never.com",
        sender="never@never.com",
    )
    db_session.add(email)
    await db_session.commit()
    await db_session.refresh(email)
