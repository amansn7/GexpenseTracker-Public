...
async def test_batch_splits_across_multiple_batches():
    ...
    mock_client.batch_classify_verbose = AsyncMock(
        return_value={
            "results": [
                LLMClassification(
                    label="expense", amount=float(i), merchant="X", category="Food", txn_date=None, confidence=0.9
                )
                for i in range(2)
            ],
            "provider": "test",
...
async def test_batch_no_session_does_not_crash():
    ...
    mock_client.batch_classify_verbose = AsyncMock(
        return_value={
            "results": [
                LLMClassification(
                    label="expense", amount=50.0, merchant="Test", category="Misc", txn_date=None, confidence=0.8
                ),
            ],
            "provider": "test",
            "model": "test-model",
...
async def test_batch_llm_bad_label_defaults_to_ignore():
    ...
    mock_client.batch_classify_verbose = AsyncMock(
        return_value={
            "results": [
                LLMClassification(
                    label="BOGUS", amount=100.0, merchant="X", category="Food", txn_date=None, confidence=0.9
                ),
            ],
            "provider": "test",
            "model": "test-model",
...
async def test_batch_log_body_snippet_600_chars():
    ...
    mock_client.batch_classify_verbose = AsyncMock(
        return_value={
            "results": [
                LLMClassification(
                    label="expense", amount=100.0, merchant="Test", category="Misc", txn_date=None, confidence=0.8
                ),
            ],
            "provider": "test",
            "model": "test-model",
    ...
    assert isinstance(log_row, ClassificationLog)
    assert len(log_row.body_snippet) == 3000
    assert log_row.body_snippet == "x" * 3000
