...
async def test_classify_email_converts_foreign_currency():
    ...
    with (
        ...
        patch("app.classifier.classifier.convert_amount", new_callable=AsyncMock, return_value=491.47),
    ):
        result = await classify_email(
            ClassificationContext(
                email_id="e-fx",
                sender="alert@sbicard.com",
...
async def test_classify_email_returns_correct_fields():
    ...
    with patch(
        ...
        new_callable=AsyncMock,
        return_value=_mock_verbose_result(
            amount=1200.0, merchant="Netflix", category="Entertainment", txn_date="2026-04-15", confidence=0.92
        ),
    ):
        result = await classify_email(
            ClassificationContext(
...
async def test_classify_email_high_confidence_is_auto():
    ...
    with patch(
        "app.classifier.classifier.llm_client.classify_verbose",
        new_callable=AsyncMock,
        return_value=_mock_verbose_result(confidence=0.95),
    ):
        result = await classify_email(
            ClassificationContext(
                email_id="e2",
...
async def test_classify_email_low_confidence_is_needs_review():
    ...
    with patch(
        "app.classifier.classifier.llm_client.classify_verbose",
        new_callable=AsyncMock,
        return_value=_mock_verbose_result(confidence=0.5),
    ):
        result = await classify_email(
            ClassificationContext(
                email_id="e3",
                sender="s@unknown.com",
