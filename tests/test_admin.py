...
async def test_admin_test_provider_uses_service_id_for_user_services(db_session):
    ...
    try:
        with patch("app.classifier.llm_client.MultiLLMClient", return_value=MagicMock(_providers=[provider], _ranked_providers=lambda: [provider])), patch("app.api._account_helpers._decrypt_secret", return_value="secret"), patch("app.classifier.llm_client.build_user_client", return_value=user_client) as build_user_client:
