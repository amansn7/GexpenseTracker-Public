    ...
    return (
      <button
        ...
        style={{
          ...
          cursor: "pointer", fontSize: "0.75rem", color: "var(--ink-2)",
      ...
      >
        ...
        <span role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleClose(); } }} onClick={(e) => { e.stopPropagation(); handleClose(); }} style={{ display:"flex", marginLeft:4, cursor:"pointer" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--ink-3)" strokeWidth="1.5" strokeLinecap="round"><path d="M6 6l12 12M18 6l-12 12"/></svg>
        </span>
      </button>
    );
  }
  ...
  return (
    <div role="status" aria-live="polite" aria-label={`Sync ${progress.phase}`} style={{
      ...
      {(progress.log || []).length > 0 && (
        <div role="log" aria-label="Sync log" style={{ margin:"10px 12px 8px", maxHeight:160, overflowY:"auto", background:"var(--paper)", borderRadius:6, padding:"6px 8px", fontFamily:"'Geist Mono',monospace", fontSize:10, lineHeight:1.7 }}>
...
(window as any).SyncProgressOverlay = SyncProgressOverlay;
