// @ts-nocheck
const Row = ({ tx, selected, selectMode, onRowClick, onCheckbox, onEditCat, onDelete }) => {
  const tag = TAGS[tx.tag];
  const isIncome = tx.amount > 0;
  const { isMobile } = useViewport();
  const [swipeX, setSwipeX] = React.useState(0);
  const [swiping, setSwiping] = React.useState(false);
  const touchStartSwipeX = React.useRef(0);
  const touchStartSwipeY = React.useRef(0);
  ...
  const handleTouchStart = (e) => {
    ...
    touchStartSwipeX.current = e.touches[0].clientX;
    touchStartSwipeY.current = e.touches[0].clientY;
  ...
  const handleTouchMove = (e) => {
    ...
    if (swipeActivated.current) {
      var dx = e.touches[0].clientX - touchStartSwipeX.current;
      if (dx < 0) {
    ...
    var ddx = e.touches[0].clientX - touchStartSwipeX.current;
    var ddy = Math.abs(e.touches[0].clientY - touchStartSwipeY.current);
    if (ddx < -10 && Math.abs(ddx) > ddy * 1.5) {
  ...
  const rowContent = (
    <div
      ...
      onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); swipedRef.current = false; e.currentTarget.click(); } }}
      onClick={handleClick}
      className={"anim-row-spring hover-bg" + (!tx.read ? " row-unread" : "")}
      style={{ ...rowStyle, ...(selected ? inboxStyles.rowSelected : {}), ...(isMobile && onDelete ? { transform: "translateX(" + swipeX + "px)", transition: swiping ? "none" : "transform 0.2s ease", position: "relative", zIndex: 1, background: "var(--card)" } : {}) }}
    >
      <div
...
const CategoryPicker = ({ current, onPick, onClose }) => {
  ...
  const content = (
    <>
      <div style={{ padding: "8px 10px 10px", fontSize: "0.6875rem", color: "var(--ink-3)", letterSpacing: "0.08em", textTransform: "uppercase", display:"flex", alignItems:"center", gap: 8 }}>
      ...
      {groups.map(g => (
        <div key={g.key}>
          ...
          {g.categories.map(item => (
            <button key={item.key} onClick={()=>onPick(item.key)} className="fade-in hover-lift" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 6, border: "none", background: current===item.key ? "var(--paper-2)" : "transparent", width: "100%", textAlign: "left", cursor: "pointer", color: "var(--ink)", fontSize: "0.8125rem" }}>
  ...
  const panel = (
    ...
    >
      ...
      <div style={inboxStyles.panelHeader}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 600, fontSize: "0.875rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tx.merchant}</div>
            <div style={{ fontSize: "0.6875rem", color: "var(--ink-3)" }}>{tx.domain}</div>
          </div>
        </div>
      ...
      <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: "0.6875rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 500 }}>{isIncome ? "Money in" : "Money out"}</div>
        {editingAmt ? (
          ...
          <div onClick={()=>setEditingAmt(true)} className="hover-border-bottom" style={{ cursor: "text", display: "inline-block" }}>
            <div style={{ ...inboxStyles.bigAmount, ...(isMobile ? { fontSize: "2.5rem" } : {}), color: isIncome ? "var(--pos)" : "var(--ink)" }}>
              {sign}₹{Math.abs(tx.amount).toLocaleString("en-IN")}
            </div>
          </div>
        )}
        <div style={{ fontSize: "0.75rem", color: "var(--ink-3)", marginTop: 4, display: "flex", alignItems: "center", gap: 6 }}>
          {dateLabel(tx.date)}: {tx.time}
          {saving && (
            <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", display: "flex", alignItems: "center", gap: 4 }}>
      ...
      <div style={inboxStyles.panelSection}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: "0.75rem", color: "var(--ink-3)", marginBottom: 10, padding: "10px 12px", background: tx.conf < 0.7 ? "var(--accent-soft)" : "var(--paper-2)", borderRadius: 6 }}>
          <Icon name="info" size={14} stroke={tx.conf < 0.7 ? "var(--accent)" : "var(--ink-3)"}/>
          <div style={{ flex: 1 }}>
            <div style={{ color: "var(--ink)", fontWeight: 500, fontSize: "0.75rem" }}>
      ...
      <div style={inboxStyles.panelSection}>
        ...
        <div style={inboxStyles.field}>
          ...
          {editingMerchant ? (
            <input
              ...
              style={{ padding: "8px 10px", border: "1px solid var(--accent)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "inherit", outline: "none", width: 140, textAlign: "right", boxSizing: "border-box" }}
            />
          ) : (
            <span onClick={()=>setEditingMerchant(true)} className="hover-border-bottom" style={{ cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4, fontSize: "0.75rem", fontWeight: 500, color: "var(--ink)", padding: "1px 0" }}>
              {tx.merchant || "\u2014"}
            </span>
          )}
        </div>
        <div style={inboxStyles.field}>
          ...
          <span style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--ink-3)", fontSize: "0.75rem" }}>
      ...
      <div style={inboxStyles.panelSection}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: "0.6875rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Email excerpt</div>
          <button onClick={handleFetchBody} style={{ fontSize: "0.6875rem", padding: "4px 10px", border: "1px solid var(--line)", borderRadius: 4, background: "transparent", color: "var(--ink-3)", cursor: "pointer", fontWeight: 500 }}>
            Fetch body
          </button>
        </div>
        <div style={{ padding: 14, background: "var(--paper-2)", borderRadius: 6, fontSize: "0.75rem", color: "var(--ink-2)", lineHeight: 1.5, border: "1px solid var(--line)" }}>
          <div style={{ fontFamily: "'Geist Mono', monospace", fontSize: "0.625rem", color: "var(--ink-4)", marginBottom: 6 }}>{tx.subject}</div>
          {fetchedBody ? (
            <>
              <div style={{ fontSize: "0.625rem", color: "var(--pos)", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>
      ...
      <div style={inboxStyles.panelSection}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: "0.6875rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Note</div>
          {saving && (
            <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", display: "flex", alignItems: "center", gap: 4 }}>
              <span className="spinner-xs" />
              Saving...
        ...
        <textarea
          ...
          style={{ width: "100%", minHeight: 60, border: "1px solid var(--line)", borderRadius: 6, padding: "8px 10px", background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", resize: "vertical", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
        />
      </div>
      ...
      <div style={inboxStyles.panelFooter}>
        {reclass === "preview" && reclassResult && editDraft && (
          <div className="fade-in" style={{ marginBottom: 10, padding: 16, background: "var(--card)", borderRadius: 12, border: "1px solid var(--line)", fontSize: "0.8125rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 16px", marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Label</div>
                <select value={editDraft.label} onChange={e=>setEditDraft(d=>({...d, label:e.target.value}))} style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}>
              ...
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Amount</div>
                <input type="number" min="0" step="0.01" value={editDraft.amount} onChange={e=>setEditDraft(d=>({...d, amount: parseFloat(e.target.value) || 0}))} style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "'Geist Mono', monospace", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Merchant</div>
                <input type="text" value={editDraft.merchant} onChange={e=>setEditDraft(d=>({...d, merchant:e.target.value}))} style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "inherit", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Category</div>
                <select value={editDraft.category} onChange={e=>setEditDraft(d=>({...d, category:e.target.value}))} style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}>
              ...
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Confidence</div>
                <div style={{ fontWeight: 500, color: "var(--ink)", marginTop: 2, fontSize: "0.75rem" }}>{Math.round((editDraft.confidence ?? 0) * 100)}%</div>
              </div>
              <div>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Date</div>
                <input type="date" value={editDraft.txn_date || ""} onChange={e=>setEditDraft(d=>({...d, txn_date:e.target.value}))} style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.75rem", fontFamily: "inherit", outline: "none", boxSizing: "border-box" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={()=>setReclass("idle")} style={{ flex: 1, padding: "10px 20px", border: "1px solid var(--line)", borderRadius: 6, background: "transparent", color: "var(--ink-2)", fontSize: "0.8125rem", fontWeight: 500, cursor: "pointer", outline: "none", transition: "background 120ms ease" }}
              ...
              <button onClick={handleConfirm} style={{ flex: 2, padding: "10px 20px", border: "none", borderRadius: 6, background: "var(--ink)", color: "var(--paper)", fontSize: "0.8125rem", fontWeight: 600, cursor: "pointer", outline: "none", transition: "background 120ms ease" }}
        ...
        {reclass === "saving" && (
          <div style={{ marginBottom: 10, padding: 10, background: "var(--paper-2)", borderRadius: 8, fontSize: "0.75rem", color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {reclass === "done" && (
          <div className="fade-in" style={{ marginBottom: 10, padding: 10, background: "var(--pos-soft)", borderRadius: 8, border: "1px solid var(--pos)", fontSize: "0.75rem", color: "var(--pos)", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {reclass === "error" && (
          <div className="fade-in" style={{ marginBottom: 10, padding: 10, background: "var(--neg-soft)", borderRadius: 8, border: "1px solid var(--neg)", fontSize: "0.75rem", color: "var(--neg)", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div onClick={() => switchMethod(reclassMethod === "llm" ? "rules" : "llm")}
          ...
          <div style={{ flex: 1, padding: "4px 8px", textAlign: "center", fontSize: "0.6875rem", fontWeight: 600, color: reclassMethod === "llm" ? "var(--paper)" : "var(--ink-3)", position: "relative", zIndex: 1 }}>LLM</div>
          <div style={{ flex: 1, padding: "4px 8px", textAlign: "center", fontSize: "0.6875rem", fontWeight: 600, color: reclassMethod === "rules" ? "var(--paper)" : "var(--ink-3)", position: "relative", zIndex: 1 }}>Rules</div>
        </div>
        <div style={{ display: "flex", gap: 8, flexDirection: isMobile ? "column" : "row" }}>
          <button className="focus-ring" onClick={()=>onUpdate({ flag: !tx.flag })} style={{ flex: 1, padding: "10px 20px", border: "1px solid var(--line)", borderRadius: 6, background: tx.flag ? "var(--accent-soft)" : "transparent", color: tx.flag ? "var(--accent)" : "var(--ink-2)", fontSize: "0.8125rem", fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          <button
            ...
            style={{ flex: 1, padding: "10px 20px", border: "1px solid var(--line)", borderRadius: 6, background: (reclass==="previewing"||reclass==="preview") ? "var(--paper-2)" : "transparent", color: (reclass==="previewing"||reclass==="preview") ? "var(--ink-4)" : "var(--ink-2)", fontSize: "0.8125rem", fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, cursor: (reclass==="previewing"||reclass==="saving"||reclass==="preview") ? "default" : "pointer" }}>
...
(window as any).Row = RowMemo;
(window as any).CategoryPicker = CategoryPicker;
(window as any).DetailPanel = DetailPanel;
