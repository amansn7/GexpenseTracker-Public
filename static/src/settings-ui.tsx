...
const DataTable = ({ columns, rows, loading, error, emptyMessage = "Nothing here yet.", emptyAction, page, total, pageSize, onPageChange }) => {
  ...
  if (loading) return <div aria-live="polite" style={{ fontSize: "0.8125rem", color: "var(--ink-3)", padding: "12px 0" }}>Loading…</div>;
  if (error) return <div role="alert" style={{ padding: 10, background: "var(--neg-soft)", color: "var(--neg)", borderRadius: 6, fontSize: "0.8125rem", marginBottom: 8 }}>{error}</div>;
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0 4px", fontSize: "0.75rem", color: "var(--ink-3)" }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            ...
            <button onClick={() => onPageChange(page + 1)} disabled={(page + 1) >= totalPages}
              style={{ padding: "4px 10px", border: "1px solid var(--line)", borderRadius: 4, background: (page + 1) >= totalPages ? "var(--paper-2)" : "var(--paper)", color: (page + 1) >= totalPages ? "var(--ink-4)" : "var(--ink-3)", fontSize: "0.6875rem", fontWeight: 500, cursor: (page + 1) >= totalPages ? "default" : "pointer" }}>Next →</button>
          </div>
        </div>
      )}
    </div>
...
(window as any).SettingsSection = SettingsSection;
(window as any).SettingsRow = SettingsRow;
(window as any).DataTable = DataTable;
(window as any).labelBadge = labelBadge;
(window as any).confColor = confColor;
(window as any).mqOverrides = mqOverrides;
(window as any).TabBtn = TabBtn;
(window as any).AdminTabBtn = AdminTabBtn;
