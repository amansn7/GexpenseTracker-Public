  ...
  const formContent = (
    <>
      ...
      <div style={{ padding: "14px 20px", borderTop: "1px solid var(--line)", display: "flex", gap: 8 }}>
        ...
        {item && confirming && (
          <Button onClick={del} style={{ background: "var(--neg)", color: "var(--paper)", border: "none" }}>Confirm Delete</Button>
        )}
        <Button onClick={onClose} style={{ marginLeft: "auto" }}>Cancel</Button>
        <Button onClick={save} disabled={saving} variant="primary">{saving ? "Saving\u2026" : "Save"}</Button>
      </div>
    </>
  );
          ...
          return (
            <div key={debt.id} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 10, padding: 18, ...(isMobile ? { padding: "12px 14px" } : {}), display: "flex", flexDirection: "column", gap: 10, position: "relative" }}>
              <div style={{ position: "absolute", top: 12, right: 12, display: "flex", gap: 4 }}>
                <Button onClick={() => setModal(debt)} variant="ghost" size="sm" style={{ padding: 4, color: "var(--ink-4)" }}>
