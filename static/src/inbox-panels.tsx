    ...
    <div style={{ flex: 1, borderRadius: 8, border: `${isPrimary ? "2px" : "1px"} solid ${isPrimary ? "var(--accent)" : "var(--line)"}`, background: isPrimary ? "var(--card)" : "var(--paper-2)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "12px 14px 10px", borderBottom: "1px solid var(--line)" }}>
        {isPrimary && (
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: "0.625rem", fontWeight: 600, color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Suggested primary</span>
          </div>
        )}
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
          <MerchantLogo merchant={tx.merchant || "?"} size={30}/>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 600, fontSize: "0.8125rem", color: "var(--ink)", lineHeight: 1.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tx.merchant || "Unknown"}</div>
            <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", marginTop: 1 }}>{tx.email?.sender_domain || ""}</div>
          </div>
          <div style={{ fontFamily: "'Geist Mono', monospace", fontWeight: 700, fontSize: "1rem", color: "var(--neg)", flexShrink: 0 }}>{fmtAmt(tx.amount)}</div>
        </div>
      </div>
      ...
      <div style={{ padding: "10px 14px", flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        {tx.txn_date && (
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.07em" }}>Date</span>
            <span style={{ fontSize: "0.6875rem", color: "var(--ink-3)", fontFamily: "'Geist Mono', monospace" }}>{tx.txn_date}</span>
          </div>
        )}
        {tx.email?.subject && (
          <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tx.email.subject}</div>
        )}
          ...
          <div style={{ marginTop: 2, padding: "8px 10px", background: "var(--paper)", borderRadius: 6, border: "1px solid var(--line)" }}>
            {fetchedBody ? (
              <>
                ...
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-2)", lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 160, overflowY: "auto" }}>{fetchedBody.slice(0, 2000)}{fetchedBody.length > 2000 ? "\u2026" : ""}</div>
              </>
            ) : (
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{tx.email?.body_snippet}</div>
                <button onClick={handleFetchBody} disabled={fetching} className="focus-ring" style={{ flexShrink: 0, fontSize: "0.625rem", padding: "2px 7px", border: "1px solid var(--line)", borderRadius: 4, background: "transparent", color: "var(--ink-3)", cursor: fetching ? "default" : "pointer", ...(isMobile ? { minHeight: 44 } : {}) }}>
                  {fetching ? "\u2026" : "Full"}
                </button>
              </div>
            )}
      ...
      <div style={{ padding: "0 14px 12px" }}>
        <button
          ...
          style={{ width: "100%", padding: "7px 0", borderRadius: 6, border: isPrimary ? "none" : "1px solid var(--line)", background: isPrimary ? "var(--ink)" : "transparent", color: isPrimary ? "var(--paper)" : "var(--ink-2)", fontSize: "0.75rem", fontWeight: 600, cursor: resolving ? "default" : "pointer", opacity: resolving ? 0.5 : 1 }}>
          Keep this
        </button>
      </div>
    </div>
  ...
  return (
    <div className="fade-in" style={{ padding: isMobile ? "16px 14px" : "18px 24px", borderBottom: "1px solid var(--line)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <span style={{ fontSize: "0.6875rem", fontWeight: 500, color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 6 }}>{reasonLabel}{isNew && <span style={{ fontSize: "0.5625rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", background: "var(--accent)", color: "white", borderRadius: 3, padding: "1px 5px 1px 5px", lineHeight: "14px" }}>New</span>}</span>
        {confidence > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <span style={{ fontSize: "0.625rem", fontFamily: "'Geist Mono', monospace", color: "var(--ink-4)" }}>{Math.round(confidence * 100)}%</span>
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 10, display: "flex", justifyContent: "flex-end" }}>
        <button
          ...
          disabled={resolving}
          className="focus-ring"
          style={{ padding: "6px 14px", border: "1px solid var(--line)", borderRadius: 6, background: "transparent", color: "var(--ink-3)", fontSize: "0.75rem", cursor: resolving ? "default" : "pointer" }}>
          Not a duplicate
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "28px minmax(0,1fr) auto" : "32px minmax(0,1fr) 130px 90px 150px 110px", gap: 12, alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--line)" }}>
      ...
      <div style={{ minWidth: 0, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontWeight: 600, fontSize: "0.8125rem", color: "var(--ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.merchant}</span>
      </div>
      <div style={{ fontFamily: "'Geist Mono', monospace", fontWeight: 600, fontSize: "0.875rem", color: "var(--pos)", textAlign: "right" }}>
        +₹{tx.amount.toLocaleString("en-IN")}
      </div>
      {!isMobile && <div style={{ fontSize: "0.75rem", color: "var(--ink-3)" }}>{dateLabel}</div>}
      {!isMobile && <div><CategoryChip cat={tx.cat}/></div>}
      {!isMobile && <div style={{ fontSize: "0.75rem", color: "var(--ink-4)" }}>{monthLabel}</div>}
    </div>
  );
};
...
const IncomeTableView = ({ transactions, onUpdate }) => {
  ...
  if (transactions.length === 0) return (
    <div style={{ padding: "40px 32px", color: "var(--ink-3)", fontSize: "0.8125rem" }}>No income transactions found for this range.</div>
  );
  ...
  return (
    <div style={{ padding: isMobile ? "0 14px 24px" : "0 32px 32px" }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "28px minmax(0,1fr) auto" : "32px minmax(0,1fr) 130px 90px 150px 110px", gap: 12, padding: "12px 0 8px", borderBottom: "2px solid var(--line)", fontSize: "0.625rem", color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 500, position: "sticky", top: 41, background: "var(--paper)", zIndex: 4 }}>
      ...
      {monthEntries.map(([monthKey, txs]) => {
        ...
        return (
          <div key={monthKey}>
            <div style={{ padding: "14px 0 6px", fontSize: "0.6875rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
...
const ReviewEmailRow = ({ email, onKeep, onDiscard, isFocused, isSelected, onFocus, onToggleSelect, previewId, onTogglePreview }) => {
  ...
  return (
    <div ref={rowRef} tabIndex={0} className="hover-bg"
      ...
      <span style={{ fontSize: "0.625rem", fontWeight: 600, color: looksLikeTx ? "var(--amber)" : "var(--ink-4)", background: looksLikeTx ? "color-mix(in srgb, var(--amber) 20%, transparent)" : "var(--paper-2)", border: `1px solid ${looksLikeTx ? "color-mix(in srgb, var(--amber) 40%, transparent)" : "var(--line)"}`, borderRadius: 4, padding: "1px 5px", textTransform: "uppercase", letterSpacing: "0.03em", flexShrink: 0 }}>
        {looksLikeTx ? "Tx" : "Noise"}
      </span>
      <span style={{ fontSize: "0.75rem", fontWeight: 500, color: "var(--ink)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{email.subject || "(no subject)"}</span>
      <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", fontFamily: "'Geist Mono', monospace", flexShrink: 0, marginRight: 4 }}>{email.sender_domain || ""}</span>
      {confidenceDots}
  ...
  return (
    <div style={{ border: "1px solid var(--line)", borderRadius: 12, marginBottom: 10, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--paper-2)", borderBottom: collapsed ? "none" : "1px solid var(--line)", cursor: "pointer", userSelect: "none" }} onClick={() => setCollapsed(!collapsed)}>
        <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", transition: "transform 120ms var(--ease-out-quart)", transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)" }}>▼</span>
        <input type="checkbox" checked={allSelected} onChange={e => { e.stopPropagation(); allSelected ? onClearSelect(emailIds) : onSelectAll(emailIds); }} onClick={e => e.stopPropagation()}
          style={{ cursor: "pointer", width: 14, height: 14, accentColor: "var(--accent)" }} />
        <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--ink)", flex: 1, fontFamily: "'Geist Mono', monospace" }}>{domain}</span>
        <span style={{ fontSize: "0.625rem", color: "var(--ink-4)", padding: "1px 6px", borderRadius: 6, background: "var(--card)" }}>{emails.length}</span>
        {(() => { const c = [...selectedIds].filter(id => emails.some(e => e.id === id)).length; if (c === 0) return null; return <span style={{ fontSize: "0.625rem", color: "var(--accent)", fontFamily: "'Geist Mono', monospace", fontWeight: 600 }}>({c})</span>; })()}
        <span style={{ fontSize: "0.625rem", padding: "1px 5px", borderRadius: 6, background: hasTxs ? "var(--accent-soft)" : "var(--paper-2)", color: hasTxs ? "var(--accent)" : "var(--ink-4)" }}>{hasTxs ? `${emails.filter(e => _looksLikeTx(e)).length} tx` : "noise"}</span>
        <div style={{ display: "flex", gap: 4 }} onClick={e => e.stopPropagation()}>
          <button onClick={handleKeepAll} disabled={busy} style={{ fontSize: "0.625rem", padding: "3px 8px", borderRadius: 6, background: "var(--accent)", color: "var(--paper)", cursor: busy ? "wait" : "pointer", fontWeight: 500, opacity: busy ? 0.6 : 1 }}>Keep all</button>
          {!confirmDiscard ? (
            <button onClick={() => setConfirmDiscard(true)} disabled={busy} style={{ fontSize: "0.625rem", padding: "3px 8px", borderRadius: 6, border: "1px solid var(--line)", background: "transparent", color: "var(--ink-2)", cursor: busy ? "wait" : "pointer", opacity: busy ? 0.6 : 1 }}>Discard all</button>
          ) : (
            <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
              {hasTxs ? (
                <span style={{ fontSize: "0.625rem", color: "var(--neg)", maxWidth: 180, lineHeight: 1.3 }}>{emails.filter(e => _looksLikeTx(e)).length} look like transactions. Discard anyway?</span>
              ) : (
                <span style={{ fontSize: "0.625rem", color: "var(--ink-3)", maxWidth: 180, lineHeight: 1.3 }}>Discard {emails.length} email{emails.length > 1 ? "s" : ""}?</span>
              )}
              <button onClick={handleDiscardAll} disabled={busy} style={{ fontSize: "0.625rem", padding: "2px 6px", borderRadius: 6, background: "var(--neg)", color: "var(--paper)", cursor: "pointer", fontWeight: 600 }}>Yes</button>
              <button onClick={() => setConfirmDiscard(false)} style={{ fontSize: "0.625rem", padding: "2px 6px", borderRadius: 6, border: "1px solid var(--line)", background: "transparent", color: "var(--ink-2)", cursor: "pointer" }}>No</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--ink)", marginBottom: 4 }}>{email.sender || email.sender_domain || "\u2014"}</div>
        {email.received_at && (
          <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", fontFamily: "'Geist Mono', monospace" }}>
            {new Date(email.received_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
          </div>
        )}
      </div>

      <div style={{ fontSize: "0.9375rem", fontWeight: 600, color: "var(--ink)", marginBottom: 16, fontFamily: "'Geist', sans-serif", lineHeight: 1.3, letterSpacing: "-0.01em" }}>
      ...
      <div style={{ padding: "10px 14px", background: "var(--paper-2)", borderRadius: 6, border: "1px solid var(--line)", marginBottom: 16, display: "flex", alignItems: "flex-start", gap: 8 }}>
        ...
        <div>
          <div style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--ink-2)", marginBottom: 2 }}>Why pending</div>
          <div style={{ fontSize: "0.6875rem", color: "var(--ink-3)", lineHeight: 1.5 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        {fullBody ? (
          <div style={{ padding: 12, background: "var(--paper)", borderRadius: 6, border: "1px solid var(--line)", fontSize: "0.75rem", color: "var(--ink-2)", lineHeight: 1.6, whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 300, overflowY: "auto" }}>
            {fullBody.slice(0, 3000)}{fullBody.length > 3000 ? "\u2026" : ""}
          </div>
        ) : (
          <div style={{ padding: 12, background: "var(--paper)", borderRadius: 6, border: "1px solid var(--line)", fontSize: "0.75rem", color: "var(--ink-4)", lineHeight: 1.5 }}>
            {email.body_snippet || "No preview available"}
            {!fullBody && (
              <button onClick={handleFetchBody} disabled={fetchingBody} style={{ marginLeft: 8, fontSize: "0.625rem", padding: "2px 8px", border: "1px solid var(--line)", borderRadius: 4, background: "transparent", color: "var(--ink-3)", cursor: fetchingBody ? "default" : "pointer" }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <button disabled={loading} onClick={() => handleAction("keep")} style={{ flex: 1, fontSize: "0.75rem", padding: "8px 16px", borderRadius: 6, border: "1px solid var(--accent)", background: "var(--accent)", color: "var(--paper)", cursor: loading ? "wait" : "pointer", fontWeight: 600, opacity: loading ? 0.6 : 1 }}>
          Keep as transaction
        </button>
        <button disabled={loading} onClick={() => handleAction("discard")} style={{ flex: 1, fontSize: "0.8125rem", padding: "10px 20px", borderRadius: 6, border: "1px solid var(--line)", background: "transparent", color: "var(--ink-2)", cursor: loading ? "wait" : "pointer", fontWeight: 500, opacity: loading ? 0.6 : 1 }}>
          Discard as noise
        </button>
      </div>
      {learned && <div style={{ fontSize: "0.6875rem", color: "var(--pos)", marginTop: 8 }}><Icon name="check" size={10} stroke="var(--pos)"/> {email.sender_domain} {learned}</div>}
    </div>
  );
};
...
(window as any).TxCard = TxCardMemo;
(window as any).DuplicatePairCard = DuplicatePairCard;
(window as any).IncomeRow = React.memo(IncomeRow, (prev, next) => prev.tx.id === next.tx.id);
(window as any).IncomeTableView = IncomeTableView;
...
(window as any).ReviewEmailRow = ReviewEmailRowMemo;
(window as any).GroupSection = GroupSection;
(window as any).ReviewDetailPanel = ReviewDetailPanel;
