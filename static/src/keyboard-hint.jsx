      ...
      {open ? (
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 6, padding: "10px 14px", boxShadow: "0 8px 24px -8px var(--shadow-lg)", minWidth: 200 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 600, color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Shortcuts</span>
          ...
          {shortcuts.map(([key, desc]) => (
            <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 0", fontSize: 11 }}>
              ...
              <kbd style={{ fontFamily: "'Geist Mono', monospace", fontSize: 10, padding: "1px 6px", background: "var(--paper-2)", borderRadius: 3, color: "var(--ink-2)", marginLeft: 12 }}>{key}</kbd>
            </div>
          ))}
        </div>
      ) : (
        <button onClick={() => setOpen(true)} aria-label="Show keyboard shortcuts" style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 20, border: "1px solid var(--line)", background: "var(--card)", color: "var(--ink-4)", fontSize: 11, cursor: "pointer", fontWeight: 500, boxShadow: "0 2px 8px -4px var(--shadow-lg)" }}>
          <Icon name="keyboard" size={13} stroke="currentColor"/>
          <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: 10, opacity: 0.7 }}>?</span>
        </button>
      )}
    </div>
  );
...
Object.assign(window, { KeyboardHint });
