...
const buildFlowSummary = (transactions, summary, catBreakdown, rangeFrom, rangeTo) => {
  ...
  const saved = summary?.saved != null ? Math.round(summary.saved) : totalIncome - totalExpenses;

  return { month: monthLabel, income, expenses, weeklyBurn, savings: Math.round(saved), totalIncome };
};

// Runtime validation helper
const validateOrThrow = (data, schema) => {
  ...
  if (!result.success) {
    ...
    console.warn(`[validateOrThrow] API response validation warning: ${issues}`);
  }
  return data;
};
...
const API = {
  ...
  get: async (path, schema) => {
    ...
    const data = await r.json();
    return schema ? validateOrThrow(data, schema) : data;
  },
...
(window as any).transformTransaction = transformTransaction;
(window as any).buildFlowSummary = buildFlowSummary;
(window as any).API = API;
(window as any).normCat = _normCat;
(window as any).catDisplay = _catDisplay;
(window as any).CategoryService = CategoryService;
(window as any).CATEGORY_GROUPS = CATEGORY_GROUPS;
(window as any).useBackgroundJob = useBackgroundJob;
(window as any).showToast = showToast;
(window as any).validateOrThrow = validateOrThrow;
(window as any)._csrfToken = () => _csrfToken;
