...
const S = {
  ...
  h1: { fontFamily: "'Geist', sans-serif", fontSize: "1.75rem", fontWeight: 400, letterSpacing: "-0.02em", margin: "2px 0 0", color: "var(--ink)" },
  sectionTitle: { fontFamily: "'Geist', sans-serif", fontSize: "1.125rem", fontWeight: 500, margin: "0 0 4px" },
  sectionSub: { fontStyle: "italic", fontFamily: "'Instrument Serif', serif", fontSize: "0.875rem", color: "var(--ink-3)", marginBottom: 18 },
  row: { display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap", marginBottom: 16 },
  label: { fontSize: "0.6875rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 500, marginBottom: 6, display: "block" },
  input: { width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.8125rem", outline: "none", fontFamily: "inherit", boxSizing: "border-box" },
  textarea: { width: "100%", padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 6, background: "var(--card)", color: "var(--ink)", fontSize: "0.8125rem", fontFamily: "inherit", outline: "none", resize: "vertical", minHeight: 120, lineHeight: 1.6, boxSizing: "border-box" },
  btn: { padding: "10px 20px", borderRadius: 6, border: "1px solid var(--line)", background: "transparent", color: "var(--ink-2)", fontSize: "0.8125rem", fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit", margin: 0, transition: "background 120ms ease" },
  ...
  result: { marginTop: 16, padding: "14px 16px", borderRadius: 6, background: "var(--paper-2)", border: "1px solid var(--line)", fontSize: "0.75rem", fontFamily: "'Geist Mono', monospace", whiteSpace: "pre-wrap", wordBreak: "break-all" },
  table: { width: "100%", borderCollapse: "collapse", fontSize: "0.8125rem" },
  th: { padding: "10px 12px", textAlign: "left", borderBottom: "1px solid var(--line)", fontSize: "0.6875rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 },
  td: { padding: "11px 12px", borderBottom: "1px solid var(--line)", verticalAlign: "middle", color: "var(--ink-2)" },
  badge: { display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 5, fontSize: "0.75rem", fontWeight: 600 },
  ...
  pill: { display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, fontSize: "0.75rem", fontWeight: 500 },
  divider: { height: 1, background: "var(--line)", margin: "20px 0" },
};

const Spinner = () => <span role="status" className="spinner" />;
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "stretch" : "center", gap: 10, marginBottom: 16 }}>
        <span style={{ fontSize: "0.75rem", color: status?.last_synced_at ? "var(--ink-3)" : "var(--ink-4)" }}>
      ...
      {progress && (
        <div style={S.progress}>
          ...
          {progress.tally && (
            <div style={{ display: "flex", gap: 10, fontSize: "0.6875rem", flexDirection: isMobile ? "column" : "row" }}>
          ...
          {progress.previews?.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: "0.625rem", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--ink-4)", marginBottom: 6, fontWeight: 500 }}>Recent</div>
              {progress.previews.slice(0, 5).map((p, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "baseline", fontSize: "0.75rem", padding: "3px 0", borderBottom: "1px solid var(--line)" }}>
          ...
          {phase === "done" && progress.result && (
            <div style={{ marginTop: 10, fontSize: "0.75rem", color: "var(--pos)", fontWeight: 500 }}>
          ...
          {phase === "error" && (
            <div style={{ marginTop: 10, fontSize: "0.75rem", color: "var(--neg)" }}><Icon name="x" size={12} stroke="var(--neg)"/> Error: {progress.error}</div>
          )}
        </div>
      )}
    </>
...
const FetchPreviewSection = () => {
  ...
  return (
    <>
      ...
      {error && <div role="alert" style={{ color: "var(--neg)", fontSize: "0.75rem" }}><Icon name="x" size={12} stroke="var(--neg)"/> {error}</div>}
              ...
              <tbody>
                {result.emails.map(e => (
                  <tr key={e.gmail_id}>
                    ...
                    <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", fontSize: "0.6875rem" }}>{e.sender_domain}</td>
                    <td style={{ ...S.td, whiteSpace: "nowrap", fontFamily: "'Geist Mono', monospace", fontSize: "0.6875rem" }}>{new Date(e.received_at).toLocaleDateString("en-IN")}</td>
                    <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", textAlign: "right" }}>{e.body_chars}</td>
                    <td style={S.td}><a href={e.gmail_link} target="_blank" rel="noreferrer" style={{ color: "var(--accent)", fontSize: "0.6875rem", textDecoration: "none" }}>Open ↗</a></td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "8px 12px", background: "var(--paper-2)", borderRadius: 6, border: "1px solid var(--line)" }}>
        <div style={{ fontSize: "0.8125rem", fontWeight: 500 }}>Use LLM</div>
        <button
          type="button"
      ...
      <div style={{ fontSize: "0.6875rem", color: "var(--ink-4)", marginBottom: 16, marginTop: -12 }}>
      ...
      <button style={{ ...S.btn, ...S.btnPrimary, width: "100%", justifyContent: "center", display: "flex", alignItems: "center", gap: 8, padding: "11px 20px", fontSize: "0.875rem" }} onClick={run} disabled={loading}>
        {loading ? <><Spinner /> Classifying…</> : "Run Classifier"}
      </button>

      {error && <div role="alert" style={{ color: "var(--neg)", fontSize: "0.75rem", marginTop: 12 }}><Icon name="x" size={12} stroke="var(--neg)"/> {error}</div>}

      {result && (
        <div style={{ marginTop: 16 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
            <span style={{ ...S.badge, ...labelColor(result.label), fontSize: "0.8125rem", padding: "4px 12px" }}>
              {result.label.toUpperCase()}
            </span>
            {result.amount != null && (
              <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: "1.375rem", fontWeight: 400 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 10 }}>
            {[
              ...
              <div key={k} style={{ padding: "10px 12px", background: "var(--paper-2)", borderRadius: 6, border: "1px solid var(--line)" }}>
                ...
                <div style={{ fontSize: "0.8125rem", fontWeight: 500, color: "var(--ink)" }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {!loading && data && (
        <>
          {data.providers.length === 0 ? (
            <div style={{ fontSize: "0.8125rem", color: "var(--ink-4)", fontStyle: "italic", padding: "12px 0" }}>No providers configured. Users can add their own via BYOK or use the FreeLLMAPI trial.</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={S.table}>
                <thead>
                ...
                <tbody>
                  {data.providers.map((p, i) => (
                    <tr key={p.source === "custom" ? `cust-${p.service_id}` : p.name}>
                      <td style={{ ...S.td, color: "var(--ink-4)", fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{i + 1}</td>
                      <td style={{ ...S.td, fontWeight: 600 }}>
                        {p.source === "custom" ? (
                          <span style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                            {p.display_name}
                            {p.service_id === activeId && <span style={{ fontSize: "0.5625rem", padding: "1px 5px", borderRadius: 3, background: "var(--accent-soft)", color: "var(--accent)", fontWeight: 600, textTransform: "uppercase" }}>Active</span>}
                            <span style={{ fontSize: "0.5625rem", padding: "1px 5px", borderRadius: 3, background: "var(--accent-soft)", color: "var(--accent)", fontWeight: 600, textTransform: "uppercase" }}>Custom</span>
                            <span style={{ fontSize: "0.6875rem", color: "var(--ink-3)", fontWeight: 400, fontFamily: "'Geist Mono', monospace" }}>{p.model}</span>
                          </span>
                        ) : p.name}
                      </td>
                      <td style={S.td}>
                      ...
                      <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem", color: p.rate_limited_secs > 0 ? "var(--neg)" : "var(--ink-4)" }}>
                        {p.rate_limited_secs > 0 ? `${p.rate_limited_secs}s` : "—"}
                      </td>
                      <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{p.priority_score.toFixed(3)}</td>
                      <td style={{ ...S.td, color: "var(--pos)", fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{p.success}</td>
                      <td style={{ ...S.td, color: p.fail > 0 ? "var(--neg)" : "var(--ink-4)", fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{p.fail}</td>
                      <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem", color: p.error_rate > 0.1 ? "var(--neg)" : "var(--ink-3)" }}>
...
Respond with only valid JSON like: {"label":"expense","amount":499,"merchant":"Swiggy","category":"Food","confidence":0.95}`;
  ...
  return (
    <div style={S.section}>
      ...
      {result && (
        <pre style={{ marginTop: 12, fontSize: "0.6875rem", fontFamily: "'Geist Mono', monospace", whiteSpace: "pre-wrap", color: result.ok ? "var(--pos)" : "var(--neg)", background: "var(--paper-2)", padding: 10, borderRadius: 6, maxHeight: 240, overflow: "auto" }}>
  ...
  return (
    <>
      ...
      {!loading && alerts.length === 0 && (
        <div style={{ fontSize: "0.8125rem", color: "var(--pos)", fontWeight: 500, padding: "14px 16px", background: "var(--pos-soft)", borderRadius: 6, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {alerts.map((a, i) => (
        <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "14px 0", borderBottom: i < alerts.length - 1 ? "1px solid var(--line)" : "none" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: "0.8125rem", color: "var(--ink)", lineHeight: 1.5, wordBreak: "break-all" }}>{a.message}</div>
...
const FetchRangeSection = () => {
  ...
  return (
    <>
      ...
      {result && (
        <div style={{ fontSize: "0.75rem", color: "var(--pos)" }}>
...
const FilterRulesSection = () => {
  ...
  return (
    <>
      ...
      {result && (
        <div style={{ fontSize: "0.75rem", color: "var(--pos)", marginBottom: 12 }}>
          <Icon name="check" size={12} stroke="var(--pos)"/> {result.generated ?? 0} new rule{result.generated !== 1 ? "s" : ""} generated
        </div>
      )}
      {error && <div role="alert" style={{ color: "var(--neg)", fontSize: "0.75rem", marginBottom: 12 }}><Icon name="x" size={12} stroke="var(--neg)"/> {error}</div>}
      {loading && <div aria-live="polite" style={{ fontSize: "0.8125rem", color: "var(--ink-3)", padding: "12px 0" }}>Loading…</div>}
      {!loading && rules.length > 0 && (
        <table style={S.table}>
          ...
          <tbody>
            {rules.map(r => (
              <tr key={r.id}>
                <td style={S.td}><span style={{ ...S.badge, background: "var(--paper-2)", color: "var(--ink-3)", fontSize: "0.6875rem" }}>{typeLabel(r.rule_type)}</span></td>
                <td style={{ ...S.td, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{r.value}</td>
                <td style={S.td}><span style={{ ...S.badge, ...sourceColor(r.source), fontSize: "0.6875rem" }}>{r.source}</span></td>
      ...
      {!loading && rules.length === 0 && (
        <div style={{ fontSize: "0.8125rem", color: "var(--ink-3)", padding: "12px 0" }}>No filter rules found.</div>
      )}
    </>
  );
};
...
const RuleEngineSection = () => {
  ...
  const TD = { padding: "6px 10px", borderBottom: "1px solid var(--line)", fontSize: "0.8125rem", verticalAlign: "top" };
  const TH = { ...TD, fontSize: "0.625rem", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 500, borderBottom: "1px solid var(--ink-4)", textAlign: "left" };
  ...
  return (
    <>
      ...
      {generated && (
        <div style={{ padding: 10, background: "var(--pos-soft)", color: "var(--pos)", borderRadius: 6, fontSize: "0.8125rem", marginBottom: 12 }}>
          Generated {generated.count} domain rule{generated.count !== 1 ? "s" : ""}
        </div>
      )}

      <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--ink-2)", marginBottom: 6 }}>Built-in rules ({builtinRules.length})</div>
      {builtinRules.length > 0 ? (
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16, fontSize: "0.8125rem" }}>
          <thead><tr>
          ...
          <tbody>
            {builtinRules.map((r, i) => (
              <tr key={i}>
                <td style={{ ...TD, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{r.domain}</td>
                <td style={TD}><span style={{ fontSize: "0.625rem", padding: "1px 6px", borderRadius: 3, ...labelColor(r.label) }}>{r.label}</span></td>
        ...
        <div style={{ fontSize: "0.75rem", color: "var(--ink-4)", fontStyle: "italic", marginBottom: 16 }}>No built-in rules</div>
      )}

      <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--ink-2)", marginBottom: 6 }}>Learned from data ({learnedRules.length})</div>
      {learnedRules.length > 0 ? (
        ...
        <div style={{ fontSize: "0.75rem", color: "var(--ink-4)", fontStyle: "italic", marginBottom: 16 }}>No learned rules yet. Use "Generate from data".</div>
      )}

      <details>
        <summary style={{ fontSize: "0.75rem", color: "var(--ink-3)", cursor: "pointer", userSelect: "none", padding: "4px 0" }}>
          ...
          {senderRules.length > 0 ? (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8125rem" }}>
              ...
              <tbody>
                {senderRules.map(r => (
                  <tr key={r.id}>
                    <td style={{ ...TD, fontFamily: "'Geist Mono', monospace", fontSize: "0.75rem" }}>{r.sender_domain}</td>
                    <td style={TD}><span style={{ fontSize: "0.625rem", padding: "1px 6px", borderRadius: 3, ...labelColor(r.label) }}>{r.label}</span></td>
                    <td style={TD}>{r.category || "—"}</td>
                    <td style={{ ...TD, fontSize: "0.6875rem", color: "var(--ink-4)" }}>{r.created_at ? new Date(r.created_at).toLocaleDateString() : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ) : (
            <div style={{ fontSize: "0.75rem", color: "var(--ink-4)", fontStyle: "italic" }}>No user corrections yet.</div>
          )}
        </div>
      </details>
    </>
...
const AdminView = () => {
  ...
  <div style={{ ...S.page, ...mqP }}>
    <div style={S.header}>
      ...
      <h1 style={{ ...S.h1, ...(isMobile ? { fontSize: "1.375rem" } : {}) }}>Admin</h1>
    </div>
    ...
    <div style={{ ...S.section, ...mqS }}><LLMStatusSection account={(window as any).currentAccount} settings={(window as any).currentSettings} /></div>
    <div style={{ ...S.section, ...mqS }}><LLMTestSection account={(window as any).currentAccount} /></div>
...
Object.assign((window as any), { AdminView, SyncSection, FetchPreviewSection, ClassifyTestSection, LLMStatusSection, LLMTestSection, AlertsSection, FetchRangeSection, RuleEngineSection, CleanBodiesSection, BackfillBodiesSection });
