const Input = ({ label, error, hint, prefix, type = "text", value, onChange, placeholder, style, className = "", id, ...props }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0, ...style }} className={className}>
      ...
      {error ? (
        <span style={{ color: "var(--neg)", fontSize: 11, marginTop: 4, fontFamily: "'Geist', sans-serif" }}>
          {error}
        </span>
      ) : hint ? (
        <span style={{ color: "var(--ink-3)", fontSize: 11, marginTop: 4, fontFamily: "'Geist', sans-serif" }}>
          {hint}
        </span>
      ) : null}
    </div>
