...
const Confidence = ({ value }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }} title={`AI confidence ${pct}%`}>
      ...
      <span style={{ fontSize: "0.625rem", fontFamily: "'Geist Mono', monospace", color: low ? "var(--accent)" : "var(--ink-4)", minWidth: 26 }}>{pct}%</span>
    </div>
  );
};
...
const SkeletonRow = () => {
  ...
  if (isMobile) {
    return (
      <div style={{ display: "grid", gridTemplateColumns: "auto 26px minmax(0, 1fr) auto", gap: 8, alignItems: "center", padding: "12px max(14px, env(safe-area-inset-right, 0px)) 12px max(14px, env(safe-area-inset-left, 0px))", borderBottom: "1px solid var(--line)" }}>
...
(window as any).fmtMoney = fmtMoney;
(window as any).groupByDate = groupByDate;
(window as any).dateLabel = dateLabel;
(window as any).MerchantLogo = React.memo(MerchantLogo);
(window as any).CategoryChip = React.memo(CategoryChip);
(window as any).Confidence = React.memo(Confidence);
(window as any).FilterChip = React.memo(FilterChip);
(window as any).SkeletonRow = React.memo(SkeletonRow);
